...
/*function r2leg()
    ...
    inherit(shape, r2leg, rectangular_strip, triangle_strip)
        r2leg.prototype.populate=function()
        {
            
        };*/
//CUSTOMSHAPE 1
function upper_leg( num, points_transform )
	{
		triangle_strip.call(this);	
		if( !arguments.length) return;	// Pass no arguments if you just want to make a dummy object that inherits everything, for making static calls	
		this.populate( this, num, points_transform );
		this.init_buffers();
	}
inherit(upper_leg, triangle_strip);

	upper_leg.prototype.populate = function( recipient, num, points_transform )	
				{	
					var vertices = [];
					var leg_indices = [];
					var offset = recipient.vertices.length;		var index_offset = recipient.indices.length;
					var topIdx = 0; var bottomIdx = num;
						
					for( var i = 0; i < num; i++ )
					{
						vertices[topIdx] 	= vec3( Math.cos(1.4* Math.PI * topIdx / num), Math.sin(1.4* Math.PI * topIdx / num), .5 );	 //Vertices only go 1.4*Pi*Radius; only want semicircle
						recipient.texture_coords[topIdx + offset]    = vec2(0, topIdx / num );
						vertices[bottomIdx] = vec3( Math.cos(1.4* Math.PI * topIdx / num), Math.sin(1.4* Math.PI * topIdx / num), -.5 );  //Vertices only go 1.4*Pi*Radius; only want semicircle			
						recipient.texture_coords[bottomIdx + offset] = vec2(1, topIdx / num );
						leg_indices.push(topIdx++);
						leg_indices.push(bottomIdx++);
					}
					
                    //Close off the portion of the circle you need to
                    leg_indices.push( 0, num, null);
                    for( var i = 1; i < num - 2; i++ )
                    {
                        leg_indices.push(1, num - i, num - i - 1)
                    }
                    
                    //strip_indices.push(0, num, null,   1, 9, null,    1, 8, null,    1, 7, null,     1, 6, null,    1, 5, null,     1, 4, null,    1, 3, null);
                    
									
					this.init_from_strip_lists(recipient, vertices, leg_indices);
